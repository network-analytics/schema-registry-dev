...
public class KafkaYangCborSerdeTest {
  ...
  private static KafkaYangCborSerde<YangDataDocument> createConfiguredSerdeForRecordValues() {
    SchemaRegistryClient schemaRegistryClient = new MockSchemaRegistryClient(Collections.singletonList(new YangSchemaProvider()));
  ...
  private YangDataDocument getRecord() {
    ...
    try {
      YangSchemaContext schemaContext = YangYinParser.parse(this.getClass().getClassLoader().getResource("test.yang").getFile());
  ...
  public void shouldRoundTripRecords() throws Exception {
    ...
    YangDataDocument roundTrippedRecord = serde.deserializer().deserialize(
  ...
  public void shouldFailWhenInstantiatedWithNullSchemaRegistryClient() {
    assertThrowsExactly(IllegalArgumentException.class, () -> new KafkaYangCborSerde<>((SchemaRegistryClient) null));
  }
